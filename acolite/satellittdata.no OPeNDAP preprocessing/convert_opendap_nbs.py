...
def convert_opendap_nbs(ncf,
                        ...
                        # or subset directly [y0, x0, y1, x1] (counted from top left corner)
                        sub=None,
                        ...
                        local=False,  # whether the input file is local or remote
                        verbosity=0):
    ...
    try:
        old = 'CF-1.4' in nc.Conventions
        if old:
            level_1c = nc.variables['Level_1C_User_Product_metadata']
            for att in level_1c.ncattrs():
                gatts[att] = level_1c.getncattr(att)
    ...
    metadata['OBASE'] = '{}_{}_{}'.format(metadata['SATELLITE_SENSOR'], dt.strftime(
        '%Y_%m_%d_%H_%M_%S'), metadata['TILE_CODE'])
    ...
    if (os.path.exists(ncfile)) & (not override):
        if verbosity > 0:
            print('File exists {}'.format(ncfile))
        nc.close()
        return(ncfile)

    # store the original attributes
    for k in gatts:
        metadata['{}_{}'.format('ext', k)] = gatts[k]
    ...
    if add_angles:
        required += [{'in': 'sun_azimuth', 'out': 'saa'},
                     {'in': 'sun_zenith', 'out': 'sza'}]

    for ib, band in enumerate(metadata['BAND_NAMES'].split(',')):
        wk = 'WAVES'
        if wk not in metadata:
            wk = 'WAVES_ALL'
        required.append(
            {'in': band, 'out': 'rhot_{}'.format(metadata[wk][ib])})
        # per band angles
        ...
        if add_angles:
            required.append({'in': 'view_azimuth_{}'.format(
                band), 'out': 'vaa_{}'.format(band)})
            required.append({'in': 'view_zenith_{}'.format(
                band), 'out': 'vza_{}'.format(band)})
    ...
    if old:
        proj4_string = nc.variables['projection'].proj4_string
    else:
        proj4_string = nc.variables['UTM_projection'].proj4_string

    pixelsize_ = (yscene[1]-yscene[0]) / \
        (sdim[0]-1), (xscene[1]-xscene[0]) / (sdim[1]-1)
    ...
    if (sub is None) and (limit is not None):
        ...
        if False:
            ...
            xrange_raw, yrange_raw = p(
                [limit[1], limit[3]], [limit[0], limit[2]])
    ...
    if sub is not None:
        ...
        if verbosity > 0:
            print('Subsetting scene to {}'.format(sub))
    else:
        if verbosity > 0:
            print('Processing full scene')
    ...
    if geometry == 'mid':
        ...
        vza_ = {b: float(nc.variables['view_zenith_{}'.format(
            b)][0, midy, midx].data) for b in metadata['BAND_NAMES'].split(',')}
        vaa_ = {b: float(nc.variables['view_azimuth_{}'.format(
            b)][0, midy, midx].data) for b in metadata['BAND_NAMES'].split(',')}
    ...
    if geometry == 'mean':
        if sub is None:
            ...
            vza_ = {b: np.nanmean(nc.variables['view_zenith_{}'.format(
                b)][0, :, :].data) for b in metadata['BAND_NAMES'].split(',')}
            vaa_ = {b: np.nanmean(nc.variables['view_azimuth_{}'.format(
                b)][0, :, :].data) for b in metadata['BAND_NAMES'].split(',')}
        else:
            sza_ = np.nanmean(
                nc.variables['sun_zenith'][0, sub[0]:sub[2], sub[1]:sub[3]].data)
            saa_ = np.nanmean(
                nc.variables['sun_azimuth'][0, sub[0]:sub[2], sub[1]:sub[3]].data)

            vza_ = {b: np.nanmean(nc.variables['view_zenith_{}'.format(
                b)][0, sub[0]:sub[2], sub[1]:sub[3]].data) for b in metadata['BAND_NAMES'].split(',')}
            vaa_ = {b: np.nanmean(nc.variables['view_azimuth_{}'.format(
                b)][0, sub[0]:sub[2], sub[1]:sub[3]].data) for b in metadata['BAND_NAMES'].split(',')}
    ...
    for r in required:
        ...
        if dsi not in datasets:
            if verbosity > 0:
                print('{} not found in {}'.format(dsi, ncf_))
            continue
        else:
            if verbosity > 0:
                print('Reading {} and writing {}'.format(dsi, dso))
            dim = nc.variables[dsi].shape

            # read dataset attributes
            atti = {attr: getattr(nc.variables[dsi], attr)
                    for attr in nc.variables[dsi].ncattrs()}
            atto = None
            ...
            if ('Sentinel-2' in metadata['SATELLITE']) & ('rhot_' in r['out']):
                if old:
                    ...
                    dat = dat.astype(np.float32) / \
                        float(gatts['QUANTIFICATION_VALUE'])
            ...
            ac.output.nc_write(
                ncfile, dso, dat, attributes=metadata, dataset_attributes=atto, new=new)
            ...
            if 'vaa_' in dso:
                if verbosity > 0:
                    print('Writing {}'.format(dso.replace('vaa_', 'raa_')))
                raa = np.abs(saa-dat)
                raa[raa > 180] -= 180
                ac.output.nc_write(ncfile, dso.replace(
                    'vaa_', 'raa_'), raa, dataset_attributes=atto)
    ...
    if add_angles:
        if verbosity > 0:
            print('Creating average view geometry')
        vza_ave /= n_vza
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--output', help='Output directory, if not provided will output to current working directory', default=None)
    parser.add_argument(
        '--limit', help='4 element for cropping ROI in coordinates (default=None)', default=None)
    parser.add_argument(
        '--sub', help='4 element for cropping ROI in pixels (default=None)', default=None)
    parser.add_argument(
        '--geometry', help='Use single geometry for the scene, either "mid" or "mean" for scene/ROI (default=mid)', default="mid")
    parser.add_argument(
        '--add_angles', help='Read and store the resolved geometry (default=False)', default=False)
    parser.add_argument('--verbosity', help='Verbosity (default=0)', default=0)
    parser.add_argument(
        '--override', help='Overwrite NetCDF files (default=True)', default=True)

    parser.add_argument(
        '--acolite_path', help='Path to ACOLITE code (if not already in Python path) (default=None)', default=None)
    ...
    ##
    if type(args.override) == str:
        args.override = eval(args.override)
    if type(args.add_angles) == str:
        args.add_angles = eval(args.add_angles)
